...
function rangeOfInteger(min: number, max: number | {}) {
  if (max == MAX || max === undefined) max = 2147483647;
  if (min === undefined) min = -2147483648;
...
function octetString(max: number) {
...
function collection(group: string, name: string):any
function collection(group: Record<string, any>, name?:any ):any
